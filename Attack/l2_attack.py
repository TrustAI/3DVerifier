...
LEARNING_RATE = 0.001     # larger values converge faster to less accurate results
...
class CarliniL2:
    ...
    def attack(self, imgs, targets):
        ...
        for i in range(0,len(imgs),self.batch_size):
            ...
            m,d= self.attack_batch(imgs[i:i+self.batch_size], targets[i:i+self.batch_size],i)
            if np.isnan(m).sum() >0:
                pass
            else:
                r.extend(m)
    ...
    def attack_batch(self, imgs, labs,index):
        f = open('./pgdlayer20-2.txt', "a+")
        print('start index is',index,file = f)
        """
        ...
        print('o_bestl2is',np.sqrt(o_bestl2),file = f)
        print('o_bestl2is',np.sqrt(o_bestl2))
        return np.array(batch - o_bestattack),o_bestl2
def pgd_attack(self,clean_images,tar):
        ...
        for i in range(10):
            ...
            for j in range(1000):
                ...
                perturbation = 0.0001 * tf.sign(gradients)
