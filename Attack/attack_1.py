...
parser.add_argument('--lr_attack', type=float, default=0.001, help='learning rate for optimization based attack')
...
def attack_1(sess,model,imgs, targets):
            ...
            other = tf.reduce_max((1-labels_pl)*pred - (labels_pl*100),1)
            ...
            for i in range(0,len(imgs),BATCH_SIZE):
                ...
                m ,d= attack_one_batch(sess,ops,imgs[i:i+BATCH_SIZE], targets[i:i+BATCH_SIZE])
                if d == 10000000000.0:
                    pass
                else:
                    r.extend(m)
...
def attack_one_batch(sess,ops,attacked_data,attacked_label):
